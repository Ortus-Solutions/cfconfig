...
component accessors=true extends='cfconfig-services.models.BaseConfig' {
	...
	private function readDebugging( debugging ) {
		...
		if (!isNull(debugging.XmlChildren) ){
			...
			for(var entry in debugging.XmlChildren) {
				...
				var stEntry = {
					"id": entry.XMLAttributes['id']?:"", //generated by Lucee
					"fullname":entry.XMLAttributes['fullname']?:"", 
					"iprange":entry.XMLAttributes['iprange']?:"",
					"label":entry.XMLAttributes['label'], //required
					"path":entry.XMLAttributes['path']?:"",
					"type":entry.XMLAttributes['type']?:"",
				}
				
				
				stEntries[entry.XMLAttributes[ 'label' ]] = stEntry;
			}
			setDebuggingEntries(stEntries);
		}
	...
	*/
	...
	private function writeDebugging( thisConfig ) {
		...
		// Write the templates
		var entries = getDebuggingEntries();
